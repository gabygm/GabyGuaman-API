...
export const mapToDTO = async (repos) => {
  const repositories: MetricsRepositoryDto[] = [];
  for (let i = 0; i < repos.length; i++) {
    const repo = repos[i];
    const verificationState = await getRepoVerificationState(repo.id);
    const metricsRepositoryDto: MetricsRepositoryDto =
      new MetricsRepositoryDto();
    ...
    metricsRepositoryDto.verificationState =
      verificationState == 604
        ? 'Verificado'
        : verificationState == 605
        ? 'En espera'
        : 'Aprobado';
    metricsRepositoryDto.state =
      repo.state == 'E' ? 'Enable' : repo.state == 'D' ? 'Disable' : 'Archived';
